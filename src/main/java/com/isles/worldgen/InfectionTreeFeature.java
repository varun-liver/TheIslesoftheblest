...
public class InfectionTreeFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        // Randomly pick one of the three structures
        ...
        Optional<StructureTemplate> template = manager.get(ResourceLocation.fromNamespaceAndPath(blest.MODID, selectedStructure));
        ...
        if (template.isPresent()) {
            ...
            if (level.getBlockState(pos.below()).is(blest.infection.get()) || level.getBlockState(pos.below()).is(blest.infection_grass.get())) {
                template.get().placeInWorld(level, placementPos, placementPos, settings, context.random(), 2);
                return true;
            }
        }
        return false;
