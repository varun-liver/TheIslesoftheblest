...
public class TheInfectionEntity extends Monster {
    ...
    public TheInfectionEntity(EntityType<? extends Monster> type, Level level) {
        super(type, level);
    }
    ...
    private static class TheInfectionMeleeAttackGoal extends MeleeAttackGoal {
        private final TheInfectionEntity guardian;

        public TheInfectionMeleeAttackGoal(TheInfectionEntity guardian, double speedModifier, boolean followingTargetEvenIfNotSeen) {
        ...
        protected void checkAndPerformAttack(LivingEntity target, double distToEnemySqr) {
            if (distToEnemySqr <= this.getAttackReachSqr(target) && this.getTicksUntilNextAttack() <= 0 && !this.guardian.isPerformingCustomAttack()) {
